...
test('packs, installs, and executes the actual npm tarball', async () => {
  ...
  try {
    ...
    const { resolveWindowsAppSdkContract } =
      await import('../scripts/windows-app-sdk.mjs');
    ...
    const consume = await runProcess(
      ...
      [
        ...
        [
          ...
          "const contract = JSON.parse(fs.readFileSync(path.join(root, 'dist', 'windows-app-sdk.runtime.json'), 'utf8'));",
          `assert.deepEqual(contract, ${JSON.stringify({
            packageVersion: expectedContract.packageVersion,
            release: {
              major: expectedContract.major,
